...
class WinningStrategy {
  ...
  private static final int NOUGHT_WON = 5;
  private static final int CROSS_WON  = 50;
  ...
  public Square.State getWinner() {      //here is for 5 to win
    for (int i = 0; i < 5; i++) {
      int score = 0;
      for (int j = 0; j < 5; j++) {
        score += valueOf(i, j);
  ...
  public boolean isDrawn() {        //still 5
    for (int i = 0; i < 5; i++) {
      for (int j = 0; j < 5; j++) {
